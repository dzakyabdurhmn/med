# Textarea — TypeUI · Warm

> **TypeUI · Warm** — multi-line text: plain, editor chrome, comment box, and chat input.
> Depends on: `input-field.md`, `buttons.md`, `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`

A textarea is the Warm **field shell** from `input-field.md` grown to multiple lines — same contrasting `neutral-tertiary` (`#F3F2E7`) fill with a `default` border, flat (`elevation-none`), and brand focus ring, with `rows` setting the min height — but its corner is **capped at `radius-md` (2px)**, the slight panel softening over the otherwise sharp shell. Richer patterns (WYSIWYG, comment box, chat) wrap the field in a single near-sharp composite card: the inner textarea goes borderless and the **focus ring moves to the wrapper**, so the whole composite lights up as one control rather than ringing a box inside a box.
...
| Resize | Warm resize allowed unless layout forbids |
...
| Part | Token / value |
|---|---|
| Outer wrapper | `default` (`#E3E2D8`) border, `radius-md`, `neutral-tertiary` (`#F3F2E7`) contrasting fill, flat (`elevation-none`) |
...
| Toolbar dividers | Warm `default-medium` between button groups on wide viewports |
...
| Text control | Flex-grow; single-line height with `rows="1"`; shell on a contrasting `neutral-tertiary` (`#F3F2E7`) fill with a `default` border, `radius-none`, padding `spacing-2-5` `spacing-3` |
