...
> **TypeUI · Warm** — horizontal slider input.
> Depends on: `colors.md`, `radius.md`, `spacing.md`, `typography.md`

The range is one of Warm's naturally round controls: a fully pill (`radius-full`) `neutral-quaternary` track carrying a circular `brand` thumb, with the same **`brand-medium` focus ring** the rest of the system uses. It stays calm and monochrome — neutral track, brand thumb and fill — and horizontal only. The thumb always shows a visible focus indicator; the track never goes rainbow.
...
**The filled track MUST follow the thumb.** The active portion of the track is painted from the control's *current value* (bind it to a CSS custom property the track's gradient reads, updated on every input event). A hardcoded fill percentage is a bug: the fill runs past the thumb and the dot reads as sitting *behind* the active bar. The thumb also paints **above** the track, never under it.
