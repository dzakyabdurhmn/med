...
| **Badge** | Optional chip label in announcement variant |
...
Block alerts and the inline announcement are **crisp near-sharp panels** (`radius-md`, 2px) — like buttons and inputs.

| Variant | Border | Radius |
|---|---|---|
| **Default** | 1px tonal — the fill, ~20% darker | `radius-md` |
...
| **Announcement** | 1px intent border on all sides | `radius-md` (2px, near-sharp) |
...
| Announcement (inline chip) | `elevation-none` |
...
Soft intent background with a **tonal border** — the fill, ~20% darker; no icon required.
...
| Badge | Intent medium/soft fill; `font-size-sm`; `font-weight-medium`; `radius-md` (2px); padding `spacing-0-5` vertical, `spacing-2` horizontal |
...
- **No off-scale radius** — every alert, block or announcement, is near-sharp (`radius-md`, 2px); nothing rounds further.
...
## Border — a tonal edge on every alert

Every alert carries a **1px border in the same shade as its own fill, just darker** — `border: 1px solid color-mix(in srgb, <the alert fill> 80%, currentColor)`: the fill mixed **20% toward its own same-hue foreground colour** (never toward grey or black), so the edge stays the alert's colour and simply reads a step darker. Each intent derives the edge from **its own** fill (info from `brand-softer`, success from `success-soft`, danger from `danger-soft`, warning from `warning-soft`), never a foreign line. The `bordered` variant stays the heavier option (a full intent border).
