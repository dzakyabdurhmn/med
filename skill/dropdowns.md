# Dropdowns — TypeUI · Warm

> **TypeUI · Warm** — transient menus for actions, navigation, filters, and selectors.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `buttons.md`

A dropdown is a floating menu anchored to a trigger — never a permanent navigation rail. The Warm panel is a crisp **near-sharp** (`radius-lg`, 2px) surface on `neutral-primary` — carrying a **subtle derived border and a medium drop shadow** (`elevation-2`), reading as a *white panel floating over the section* (it sits above the page, below modals). Items sit inside at the soft control corner `radius-md` (2px) — a menu row is a hoverable control, so it takes the control radius, matching the panel's own near-sharp geometry (see `radius.md`). Menus open on click by default, trap nothing, and return focus to the trigger on close.
...
Menu items take the soft control corner `radius-md` (2px) — **never** `radius-xs`. A menu row is a control the user hovers and clicks, not a decorative inset cell, so it wears the same crisp 2px corner as every other Warm control, matching the panel's own near-sharp geometry (see `radius.md`).

| Property | Token / value |
|---|---|
| Padding | `spacing-2` |
| Radius | `radius-md` (2px) — the soft control corner, never `radius-xs` |
...
font-size-sm, font-weight-medium, `body-subtle`; padding `spacing-2`; not hoverable.
...
Menu **width is always auto** — sized to the longest item's text within the ~144–320px clamp; only the item padding changes by size. The trigger follows the `buttons.md` base size unless it's icon-only — and in **dashboard / widget chrome it takes the exact Small button box (~38px, `line-height: 1` + padding construction)** so it sits flush with the Small buttons beside it; a trigger never renders at a different height than an adjacent button (see the same-height row rule in `SKILL.md`). **A labeled trigger also wears the full button label voice — uppercase, `font-weight-normal` (400), the one 12px label size, `letter-spacing-wide`** — never its own private weight, size, or casing; visually it *is* a button that happens to open a menu.
...
A row of checkbox + label for multi-select; the checked state uses the brand accent on the control. The checkbox box stays a **16 × 16px box** (`radius-xs`, 2px) — never fully round or pill-shaped, even inside a filter panel.
...
- **No arrow or chevron icon beside another icon** — a dropdown chevron (and any directional arrow glyph) is only ever placed next to a **text label**, never next to another icon. When a trigger collapses to icon-only (e.g. it drops its label on a small screen), the chevron is dropped with it — never leave an arrow floating beside a lone glyph.
...
- **No round checkboxes in menus** — filter / checkbox menu rows use the near-sharp tick box (`radius-xs`, 2px), never a pill or circle. See `checkbox.md`.
...
- **No near-square menu rows** — menu / nav items take the soft control corner `radius-md` (2px), never `radius-xs`. A sharp-cornered row inside a soft 12px panel breaks the Warm silhouette.
