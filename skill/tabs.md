...
> **TypeUI · Warm** — section switching and supplementary navigation.
> Depends on: `colors.md`, `radius.md`, `spacing.md`, `typography.md`

Tabs split content into mutually exclusive panels or route-level navigation. Warm ships three flavors — a soft-filled **default**, a clean **underline**, and **pills** — and the active tab always speaks in `fg-brand`. The default and pill variants take the near-sharp **`radius-md`** (2px) shell; underline stays minimal with a 2px brand rule. Keep tab bars short and single-level; past ~7 tabs, move to an overflow menu.
...
The active tab keeps `radius-md` (2px) top corners; the list carries a `default` bottom border.
...
Inactive/hover and active pills both use `radius-md` (2px); the active pill takes the brand fill.
