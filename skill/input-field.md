# Input Field — TypeUI · Warm

> **TypeUI · Warm** — the field shell every text control inherits.
> Depends on: `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`, `dropdowns.md`

This is the foundation of every form in Warm. The field shell is a crisp **near-sharp** (`radius-md`, 2px) control on a **contrasting `neutral-tertiary` fill** (`#F3F2E7`, a step darker than the card / section it sits on), defined by a **`default` (`#E3E2D8`) border** with **no shadow** (fields are flat, like buttons), and — on focus — a `brand` border under a `brand-medium` ring. Labels sit above in `heading`; helper and validation text sit below. Search, number, phone, file, select, and time controls all extend *this* shell, so it stays consistent unless a module explicitly overrides it. The multi-line **`textarea`** extends it too but **caps its radius at `radius-md` (2px)** — the same crisp corner as the shell.
...
|---|---|
| Width | 100% of parent |
| Background | `neutral-tertiary` (`#F3F2E7`) — a contrasting fill, a step darker than the card/section |
| Border | `default` (`#E3E2D8`), 1px — the resting edge that defines the field |
| Radius | `radius-md` (2px) — inputs are crisp near-sharp rectangles, like buttons |
| Shadow | **`elevation-none`** — fields are flat, like buttons |
| Text color | `heading` |
...
### Focus ring — soft halo, legible border (mandatory)

A focused field wears **two layers**, never one:

1. **Border** — the load-bearing focus indicator. It darkens to `fg-brand` and must clear **3:1** against the field fill (it does: 5.2:1). This is what a keyboard user actually reads to know where they are.
2. **Ring** — a **soft** `brand-soft` halo (`--focus-ring-input`), the gentle glow only. It is decorative and is *not* counted toward the 3:1.

**Never make the ring the sole indicator.** A soft-crimson ring alone is **~1.1:1** against the page — invisible; a field that lights up with a halo nobody can see is not focused, it is broken. And never focus a field with the *bright* brand fill on its border plus a dark ring: that pairing reads as an error alarm, not a text box waking up. Soft halo, legible border, every time.
...
| Row border | `default` (`#E3E2D8`) on the outer perimeter |
...
| Addon border | A `default` (`#E3E2D8`) divider on the shared edge separates addon from control |
...
|---|---|---|---|---|
| Default | `neutral-tertiary` (`#F3F2E7`) | `default` (`#E3E2D8`) | `heading` / `body` | `brand-medium` |
...
- **No corners other than `radius-md`** (2px) on the shell — a fused input group keeps 2px on its **outer** corners only; shared inner edges stay square so the pieces read as one control.
- **No shadow at all (`elevation-none`)** — the field is flat like every control; never a lift, a heavier shadow, or a colored shadow.
- **No same-as-surface fill** — the field uses the contrasting `neutral-tertiary` (`#F3F2E7`) fill plus the `default` (`#E3E2D8`) border so it reads on the light surface; focus swaps the border to a `brand` edge.
