...
The near-sharp (2px) corner lives on the outermost shell; interior items square off so the rounding reads only at the edges of the group.

### Variant: grouped (default)

| Element | Border | Radius |
|---|---|---|
| Root | None — lighter card surface defines the group | `radius-md` (2px) on outer corners |
...
- **No outer shell above `radius-md`** — the near-sharp (2px) frame is the signature; interior items square off, they never round further.
