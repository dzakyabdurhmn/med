...
| Outer radius | `radius-md` (2px) on first/last segment outer corners only |
...
Warm groups follow the same rules with top/bottom outer radius on the first/last segments and the negative margin stacked on the block axis.
...
**Segment box (mandatory).** Every segment in a group shares one height: **42px**, the Base button box. A **text segment** takes 12px vertical / 16px horizontal padding inside that box. An **icon-only segment is a 42 × 42px square** — same height as the text segment, width equal to the height, with the glyph centered and no label padding. Never let an icon-only segment shrink to the size of its glyph: a group whose segments differ in height (or whose icon cells hug the icon) has stopped reading as one object, which is the whole point of the component.
