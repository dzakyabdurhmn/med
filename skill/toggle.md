...
| Track off | `neutral-tertiary` (`#F3F2E7`, contrasting) with a `default` (`#E3E2D8`) border |
...
The whole option dims to **one uniform opacity** — the track, thumb and label text share the **exact same** reduced opacity, so neither reads more disabled than the other. Apply the opacity to the option's `label` wrapper (not separately). Native `disabled` attribute, `cursor: not-allowed`, no pointer events; the thumb does not slide.
...
A bordered settings row: padding `spacing-4`, `default-medium` border, **`radius-md`**, flex space-between — copy block at the start, toggle at the inline end. Title `font-weight-medium`; description `body-subtle`. (The card is a near-sharp shell; the switch track stays `radius-full`.)
...
| Disabled | Track, thumb and label dimmed to one uniform opacity | No slide on interaction |
...
A toggle's **label text is not selectable**: apply `user-select: none` to the option's `label` wrapper, so clicking the switch (or its label) flips the toggle and never selects the label copy.
