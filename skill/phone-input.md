# Phone Input — TypeUI · Warm

> **TypeUI · Warm** — telephone entry: country code, verification codes, and auth flows.
> Depends on: `input-field.md`, `select.md`, `colors.md`, `radius.md`, `shadows.md`, `spacing.md`, `typography.md`

A phone input is the Warm **field shell** typed for `tel`. When a country selector is attached, the selector and number field **fuse into one near-sharp (`radius-md`, 2px) control** — one shared `default`-bordered, flat (`elevation-none`) shell, square inner edge — exactly like an input group. Verification (OTP) reuses the per-cell box pattern from `number-input.md`. Country flags always carry a text alternative, and OTP digits always use visible per-cell inputs.
...
| Gap (country ↔ number) | Fused group — no gap; shared `radius-md`, flat (`elevation-none`) |
...
A row of 4–6 single-digit fields (the PIN pattern from `number-input.md`): each cell a **perfect square** — equal width and height (`44 × 44px`), centered, `spacing-2` gap, **`radius-md`**, 4px `brand-medium` focus ring.
...
- **No rectangular verification cells** — every code cell is a **perfect square**: its width equals its height, never a taller-than-wide (or wider-than-tall) box.
