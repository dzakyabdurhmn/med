...
Two text sizes take the 2px corner; the numeric **count / icon badge is the round exception** — a genuinely circular bubble, always `radius-full`.

| Size | Type | Padding | Radius |
|---|---|---|---|
| **Small (default)** | Text | `spacing-1-5` horizontal, `spacing-0-5` vertical | `radius-md` (2px — always) |
| **Large** | Text | `spacing-2` horizontal, `spacing-1` vertical | `radius-md` (2px — always) |
| **Icon / count small** | Circle | — | 20 × 20px, `radius-full` (a circular count bubble — always) |
| **Icon / count large** | Circle | — | 24 × 24px, `radius-full` (always) |
...
Every variant below is a near-sharp chip — the differences are fill, border, and contents, never the corner.

### Default

Soft fill with a **tonal border** — the fill, ~20% darker (`color-mix(in srgb, <the fill> 80%, currentColor)`). Inline-flex, width fits content.
...
Every badge in Warm is **near-sharp** — `radius-md` (2px) on the root in all sizes and variants (default, bordered, link, icon, dismissible). There is no pill badge in this theme; only the inner status dot, the round count bubble, and the avatar image are circular.

### Large bordered (inset ring)

Large size with a 1px inset ring in the intent border color (`brand-subtle`, etc.) on the near-sharp `radius-md` shell — reads like bordered without doubling the outer edge.
...
- **No pill badges** — every badge is near-sharp (`radius-md`, 2px); never `radius-full` pill corners. This is the signature, not a preference. (The inner status dot, the round count bubble, and the avatar image stay circular.)
...
## Border — a tonal edge on every badge

Every badge carries a **1px border in the same shade as its own fill, just darker** — `border: 1px solid color-mix(in srgb, <the badge fill> 80%, currentColor)`: the fill mixed **20% toward its own same-hue foreground colour** (never toward grey or black), so the edge stays the badge's colour and simply reads a step darker. Each intent derives the edge from **its own** fill (success from `success-soft`, danger from `danger-soft`, warning from `warning-soft`, brand from `brand-softer`, neutral from its neutral fill), never a foreign line. The `bordered` variant stays the heavier option (a full intent-foreground border).
