...
A checkbox column at the inline start; the head checkbox selects all visible rows; each checkbox is a **16 × 16px box** (`radius-xs`, 2px) — never fully round. Selected rows may take a `brand-softer` background; a bulk-action bar appears above the table once the selection count is greater than zero.
...
**A cell link is NEVER underlined at rest.** No `text-decoration: underline` in the resting state of any `td` link — not the row title, not the ID, not a row action. A table is already a grid of scannable rows; resting underlines turn every row into visual noise and make the table look like a link dump. The link is identified by **`fg-brand` + medium weight**, and the underline appears **only on `:hover` / `:focus-visible`**. This holds even where a generic "underline links inside body copy" rule would otherwise apply — a data-table cell is not body copy, and the weight change is the resting non-color channel.
